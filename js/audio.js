...
export class AudioManager {
    ...
    playIntroMusic() {
        ...
        droneOsc.start();
        this.introNodes.push(droneOsc);

        // Gentle Arpeggiator loop (A major chord)
        ...
        const playNote = () => {
            ...
            if (this.introNodes.length > 20) {
                this.introNodes = this.introNodes.slice(this.introNodes.length - 19);
            }

            i++;
            this.introNoteTimeout = setTimeout(playNote, 1000);
        };
    ...
    stopIntroMusic() {
        ...
        if (this.introNoteTimeout) {
            clearTimeout(this.introNoteTimeout);
            this.introNoteTimeout = null;
        }

        if (this.introNodes) {
            this.introNodes.forEach(node => {
                try {
                    node.stop();
                } catch (e) { }
            });
